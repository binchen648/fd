...
Goal: migrate exactly `master.ciel.skill.s3` as the sole new frozen identity by appending it to the existing Ciel archive. Use only synchronized FB2-52 plus accepted generic master-skill play/outside-game/per-game infrastructure. S may remove only Atalanta S2 test's stale absolute `overlap===148` assertion while preserving stable Atalanta identity/denominator/duplicate invariants. No runtime changes and no s1b. Because the existing Ciel archive is already registered through `authoringMasterRuleFiles`, S must regenerate only the deterministic rules-only generated archive representation needed for executable target-definition availability; pack manifest and playable master/ordinary-card/deck/character surfaces must remain unchanged. No merge/retarget. Focused validation additionally authorizes replacing only the recovery-era Ciel s2 `raw.cards.length===2` snapshot with stable checks that s1a and s2 each remain present exactly once; no other Ciel-s2 assertion may change. Full-CI probing further authorizes only two registration snapshots: FB2-26 may relax only the Ciel `[source,target]` exact list while preserving s1a/s2 each exactly once (Ryougi/Shirou remain exact), and executable-card-pack may update only its single absolute compiled-card count from 76 to the mechanically required 77; no other compatibility edit is authorized. Required material accounting is exact `148/944 -> 149/944`; formal migration stays `153/944` until fresh independent R `MIGRATION_ACCEPTED` plus A synchronization. Long-term S rule: **S 完成 recertification 并提交 Exact Base/Candidate**。
