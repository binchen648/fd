...
| `e2e/fd-time-alter-core-primitive.spec.ts` | Missing from current checkout | Required Gate C path for `CARD_ACTION_SEMANTICS_MINIMAL_PLAY` exact `play_selected_cards` + `draw_cards` shape | restore or recreate before independent reviewer promotion; this PLAY evidence must not be inherited by `CARD_ZONE_CORE_DIRECT_ACTION` |
