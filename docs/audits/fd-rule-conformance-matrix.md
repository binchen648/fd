...
| `docs/reports/2026-09-09-resource-numeric-core-result.md` | Resource/Numeric implementation candidate: typed primitives, compiler fail-closed validation, semantic direct-action routing, and MatchSession Gate B candidate for command spell/Tomoe direct resources | Gate C REQUIRED / NOT_VERIFIED for `RESOURCE_NUMERIC_CORE_DIRECT_ACTION` | add real browser/WS/reconnect/stale evidence before any Gate C promotion; broader Resource/Numeric trigger/battle/interaction migration remains out of scope |
