...
| `e2e/fd-time-alter-core-primitive.spec.ts` | Time Alter PLAY browser/server candidate from restored remote action phase: browser ability activation, WebSocket activation/target `expectedRevision`, pending hand-card target reconnect, server target selection, selected attack projected face-down in `attack_area`, draw projection, post-settlement reconnect, stale target replay rejection, and repeat-each stability | Gate C candidate for `CARD_ACTION_SEMANTICS_MINIMAL_PLAY` exact `play_selected_cards(face_down)` + `draw_cards(1)` shape only; does not prove natural create/select/start progression into this action window | independent reviewer promotion required; this PLAY evidence must not be inherited by `CARD_ZONE_CORE_DIRECT_ACTION`, `PLAY_SOURCE_CARD_WITH_COST_RESPONSE`, Drake hidden/power/lifecycle play, add, activate, close, create-and-activate, or normal action play |
| `e2e/fd-conversion-magic-core-primitive.spec.ts` | Conversion Magic Card/Zone browser/server candidate: restored remote advance phase, server-supplied activation, WebSocket `expectedRevision`, server-authoritative missing-revision rejection coverage, actual hand-to-discard movement, mana gain from actual moved count, reconnect consistency, stale replay rejection, and repeat-each stability after same-client socket close race hardening | Gate C candidate for `CARD_ZONE_CORE_DIRECT_ACTION` exact `move_all_remaining` + bound `adjust_mana` shape only | independent reviewer promotion required; no inheritance for private look/exclusion, cost, trigger, play, add, activate, close, lifecycle, or battle-dependent card-zone abilities |
| `e2e/fd-command-spell-resource-core.spec.ts` | Command spell Resource/Numeric browser/server candidate: real remote room create/select/start, remote end-turn progression with `expectedRevision`, browser activation, WebSocket `expectedRevision`, mana gain, command seal -1, server projection resource envelopes, reconnect, and stale replay rejection | Gate C candidate for `RESOURCE_NUMERIC_CORE_DIRECT_ACTION` command spell representative only | independent reviewer promotion; broader Resource/Numeric trigger/battle/interaction migration remains out of scope |
| `e2e/fd-add-to-attack-card-action.spec.ts` | Maiya add-to-attack browser/server candidate from restored remote advance phase: browser ability activation, WebSocket activation/target `expectedRevision`, pending target reconnect, server target selection, support card projected in another player's attack area, post-settlement reconnect, stale target replay rejection, and repeat-each stability | Gate C candidate for `CARD_ACTION_SEMANTICS_MINIMAL_ADD_TO_ATTACK` exact visible `attach_card_to_player_attack` shape only; does not prove natural create/select/start progression into this action window | independent reviewer promotion required; no inheritance for normal play, create-and-activate, activate, close, non-skill source zone, support-shot suppress, round-end return cleanup, hidden/private choice, modifier, scoring, or battle-result semantics |
