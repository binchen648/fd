...
- Runtime inventory source: `docs/audits/fd-flow-runtime-inventory.md` is missing; owner mapping below is derived from source inspection and existing tests.
- Git baseline: `D:\fd` is not a Git worktree, so branch/HEAD attribution is unavailable.
...
| Battle winner | `combat-resolver.ts::resolveBattlefield`, `buildBattleResult` | `resolveBattlefield` | MatchSession battle auto-flow -> combat resolver -> ability events | Current result model is single `winnerPlayerId`; tied winners canonical requirement not fully represented |
...
| Gap | Affected Rules | Reason |
|---|---|---|
| Battle winner Gate B/C is still missing after the component fix. | FD-BATTLE-001, FD-VP-001 | Resolver, scoring, ability event, replay, and client bridge have component evidence, but no Golden Flow browser/reconnect evidence. |
