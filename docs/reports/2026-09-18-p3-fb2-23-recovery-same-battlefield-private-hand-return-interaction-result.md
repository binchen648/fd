...
## R48-R1 revision provenance

Fresh independent R48-R1 returned `IMPLEMENTATION_NEEDS_REVISION` for one blocking finding: the reserved-vocabulary semantic classifier did not close the entire supported envelope. Seven malformed structures still reported `candidate=true / semantic=true`: wrong ability kind, wrong phase, wrong opens, wrong target type, an extra effect, an extra player target, and an extra activation key.

This revision changes only the generic classifier/test surface already inside the six-file FB2-23 scope. The semantic gate now requires exact:

- `kind = phase_action`;
- activation `phase=action`, `opens=controller_action_window`, `requiresSourceState=active`, with activation-key closure;
- exactly one target and exact `target.type=player`;
- exactly one effect;
- empty lifecycle/limit/visibility and only the loader-normalized empty response-window fields;
- the previously required exact target/effect/count/constraint structure.

All seven R48-R1 adversarial mutations are now permanent regression cases and fail closed through `interaction.gateway`. Two additional envelope-closure regressions cover an extra response-window field and non-empty ability visibility. No identity/name/text/Reference routing was added.

Timing corroboration for the unrelated fixed-5s MatchSession test used the exact planning Base and revised Candidate without repository/config changes. Isolated Base timings were approximately `1721 / 1849 / 1764 ms`; revised Candidate timings were approximately `1763 / 1757 / 1753 ms`, providing no evidence of a revision-specific performance regression.
...
- typecheck: PASS;
- focused FB2-23 + existing private optional interaction: `2 files / 17 tests PASS`;
- unchanged official `npm run test:ci`: final retry `132 files / 860 tests PASS`;
- timing-sensitive eleven-round MatchSession test first hit the unchanged 5000 ms boundary at about 5111 ms, then the unchanged official retry passed at about 3963 ms; no timeout/config/test modification was made;
- rules core + regression: `72 files / 445 tests PASS`;
