...
- before requesting fresh R: typecheck, focused/task-relevant compatibility, `git diff --check`, exact scope and cleanliness. Official CI, content validation, generated determinism, exact Locked Reference verify, client build, and Phase 3 coverage/audit remain final-evidence checks for the stable Candidate; do not blanket-rerun them before every revision R unless the exact finding depends on that surface.
