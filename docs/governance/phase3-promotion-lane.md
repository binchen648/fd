...
The Phase 3 gate runs on all pull requests so stacked Phase 3 PRs get a stable policy check even when their base is another `codex/...` branch. Ordinary non-Phase 3 PRs are skipped by policy detection. The gate uses `pull_request`, not `pull_request_target`, has `contents: read`, uses no secrets, and cancels stale runs for the same PR.
