...
describe('Phase 3 full-roster capability mapping', () => {
  ...
  it('keeps the checked-in 944-identity catalog complete, fail-closed, and route-enum constrained', () => {
    ...
    expect(inventory.capabilitySummary.contractMappedCount).toBe(751);
    expect(inventory.capabilitySummary.explicitBlockCount).toBe(193);
    expect(inventory.capabilitySummary.zeroSilentFallback).toBe(true);
    expect(catalog.coverage.mappedAbilities).toHaveLength(751);
    expect(catalog.coverage.blockedAbilities).toHaveLength(193);
    ...
    expect(markdown).toContain('contractMappedCount=751');
    expect(markdown).toContain('explicitBlockCount=193');
  ...
  it('maps the ten-ID Ophelia slice with three generic extensions and seven reviewed-special event rules', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2);
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239);
    expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });
  ...
  it('maps the seventeen-ID Sion evidence batch as two generic and fifteen reviewed-special identities', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });
  ...
  it('maps the fifteen-ID Archer five batch as reviewed-special identities with shared rule families', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });
  ...
  it('maps the fifteen-ID Benkei/Billy/Boudica/Bradamante/Brynhildr batch as four generic and eleven reviewed-special identities', () => {
    ...
    expect((byId.get('servant.benkei.skill.sc-benkei-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:skill_copy_lifecycle_rule'); expect((byId.get('servant.billy.skill.sc-billy-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:repeat_hidden_play_draw_rule'); expect((byId.get('servant.brynhildr.skill.sc-brynhildr-2') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:love_bond_vp_rule'); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });
  ...
  it('maps the fifteen-ID Jaguarman/Jeanne/JeanneAlter/Jekyll/Kagekiyo/Kama batch as one reusable generic and fourteen reviewed-special identities', () => {
    const inventory=JSON.parse(readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'),'utf8')) as any;
    const entries=[...inventory.staticSkills,...inventory.dynamicSkills];
    const ids=["servant.jaguarman.skill.sc-jaguarman-1","servant.jaguarman.skill.sc-jaguarman-2","servant.jaguarman.skill.sc-jaguarman-3","servant.jeanne.skill.sc-jeanne-1","servant.jeanne.skill.sc-jeanne-2","servant.jeanne.skill.sc-jeanne-3","servant.jeanne-alter.skill.sc-jeanne-alter-1","servant.jeanne-alter.skill.sc-jeanne-alter-2","servant.jeanne-alter.skill.sc-jeanne-alter-3","servant.jekyll.skill.sc-jekyll-1","servant.jekyll.skill.sc-jekyll-2","servant.jekyll.skill.sc-jekyll-3","servant.kagekiyo.skill.sc-kagekiyo-1","servant.kagekiyo.skill.sc-kagekiyo-3","servant.kama.skill.sc-kama-3"];
    const generic=['servant.jaguarman.skill.sc-jaguarman-1']; const byId=new Map(entries.map((e:any)=>[e.canonicalAbilityId,e]));
    for(const id of generic){expect((byId.get(id) as any).phase3.classificationRoute).toBe('READY_GENERIC_EXTENSION');expect((byId.get(id) as any).phase3.blockedBy).toEqual([]);}
    for(const id of ids.filter(id=>!generic.includes(id))){expect((byId.get(id) as any).phase3.classificationRoute).toBe('SPECIAL_HANDLER_CANDIDATE');expect((byId.get(id) as any).phase3.requiredCapabilities).toContain('REVIEWED_SPECIAL_HANDLER');}
    expect((byId.get('servant.jaguarman.skill.sc-jaguarman-2') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:jaguarman_ambush_forest_rule');
    expect((byId.get('servant.jeanne.skill.sc-jeanne-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:defeat_player');
    expect((byId.get('servant.jeanne-alter.skill.sc-jeanne-alter-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:jeanne_alter_luck_avenger_rule');
    ...
    expect((byId.get('servant.kagekiyo.skill.sc-kagekiyo-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:kagekiyo_vengeance_rule');
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });

  it('maps the fifteen-ID Karna/KingGil/KingHassan/Kingprotea/Kintoki batch as reviewed-special identities', () => {
    const inventory=JSON.parse(readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'),'utf8')) as any; const entries=[...inventory.staticSkills,...inventory.dynamicSkills];
    const ids=["servant.karna.skill.sc-karna-1","servant.karna.skill.sc-karna-2","servant.karna.skill.sc-karna-3","servant.kinggil.skill.sc-kinggil-1","servant.kinggil.skill.sc-kinggil-2","servant.kinggil.skill.sc-kinggil-3","servant.kinghassan.skill.sc-kinghassan-1","servant.kinghassan.skill.sc-kinghassan-2","servant.kinghassan.skill.sc-kinghassan-3","servant.kingprotea.skill.sc-kingprotea-1","servant.kingprotea.skill.sc-kingprotea-2","servant.kingprotea.skill.sc-kingprotea-3","servant.kintoki.skill.sc-kintoki-1","servant.kintoki.skill.sc-kintoki-2","servant.kintoki.skill.sc-kintoki-3"]; const byId=new Map(entries.map((e:any)=>[e.canonicalAbilityId,e]));
    for(const id of ids){expect((byId.get(id) as any).phase3.classificationRoute).toBe('SPECIAL_HANDLER_CANDIDATE');expect((byId.get(id) as any).phase3.requiredCapabilities).toContain('REVIEWED_SPECIAL_HANDLER');}
    expect((byId.get('servant.karna.skill.sc-karna-1') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:karna_brahma_rule');
    expect((byId.get('servant.kinggil.skill.sc-kinggil-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:kinggil_cannon_rule');
    expect((byId.get('servant.kinghassan.skill.sc-kinghassan-1') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:kinghassan_death_bell_rule');
    expect((byId.get('servant.kingprotea.skill.sc-kingprotea-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:kingprotea_infinite_growth_rule');
    expect((byId.get('servant.kintoki.skill.sc-kintoki-2') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:kintoki_golden_spark_rule');
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(239); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(510);
  });
