...
describe('Phase 3 full-roster capability mapping', () => {
  ...
  it('keeps the checked-in 944-identity catalog complete, fail-closed, and route-enum constrained', () => {
    ...
    expect(inventory.capabilitySummary.contractMappedCount).toBe(313);
    expect(inventory.capabilitySummary.explicitBlockCount).toBe(631);
    expect(inventory.capabilitySummary.zeroSilentFallback).toBe(true);
    expect(catalog.coverage.mappedAbilities).toHaveLength(313);
    expect(catalog.coverage.blockedAbilities).toHaveLength(631);
    ...
    expect(markdown).toContain('contractMappedCount=313');
    expect(markdown).toContain('explicitBlockCount=631');
  ...
  it('maps the ten-ID Ophelia slice with three generic extensions and seven reviewed-special event rules', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2);
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(179);
    expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(132);
  });
  ...
  it('maps the twenty-three-ID Fujino/Ryougi/Shiki/Akiha batch as seven generic and sixteen reviewed-special identities', () => {
    const inventory = JSON.parse(readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'), 'utf8')) as any;
    const entries = [...inventory.staticSkills, ...inventory.dynamicSkills];
    const ids = [
    ...
    for (const id of ['master.fujino.skill.ascension','master.fujino.skill.s1','master.fujino.skill.s3','master.shiki-ryougi.skill.s1','master.shiki-ryougi.skill.s1a','master.shiki-ryougi.skill.s3','master.shiki-tohno.skill.s1']) {
      expect((byId.get(id) as any).phase3.classificationRoute).toBe('READY_GENERIC_EXTENSION');
      expect((byId.get(id) as any).phase3.blockedBy).toEqual([]);
    }
    const special = new Map([
      ['master.fujino.skill.s1a','SPECIAL_EFFECT:injury_rule'],['master.fujino.skill.s2','SPECIAL_EFFECT:map_adjacency_rule'],['master.fujino.skill.s4','SPECIAL_EFFECT:injury_rule'],
      ['master.shiki-ryougi.skill.ascension','SPECIAL_EFFECT:grant_linked_ability_to_attribute'],['master.shiki-ryougi.skill.s1b','SPECIAL_EFFECT:deck_bottom_rule'],['master.shiki-ryougi.skill.s2','SPECIAL_EFFECT:deck_bottom_rule'],
      ['master.shiki-tohno.skill.ascension','SPECIAL_EFFECT:control_resource_rule'],['master.shiki-tohno.skill.s1a','SPECIAL_EFFECT:control_resource_rule'],['master.shiki-tohno.skill.s2','SPECIAL_EFFECT:fusion_rule'],['master.shiki-tohno.skill.s3','SPECIAL_EFFECT:control_resource_rule'],['master.shiki-tohno.skill.s4','SPECIAL_EFFECT:fusion_rule'],
      ['master.akiha.skill.ascension','SPECIAL_EFFECT:bloodlust_rule'],['master.akiha.skill.s1','SPECIAL_EFFECT:bloodlust_rule'],['master.akiha.skill.s1a','SPECIAL_EFFECT:bloodlust_rule'],['master.akiha.skill.s2','SPECIAL_EFFECT:bloodlust_rule'],['master.akiha.skill.s3','SPECIAL_EFFECT:bloodlust_rule'],
    ]);
    for (const [id,reason] of special) { expect((byId.get(id) as any).phase3.classificationRoute).toBe('SPECIAL_HANDLER_CANDIDATE'); expect((byId.get(id) as any).phase3.blockedBy).toContain(reason); }
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2);
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(179);
  ...
  it('maps the twenty-ID Hakuno/Rani/Jinako/Alice batch as one generic and nineteen reviewed-special identities', () => {
    const inventory = JSON.parse(readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'), 'utf8')) as any;
    const entries = [...inventory.staticSkills, ...inventory.dynamicSkills];
    const ids = ['master.hakuno-f.skill.ascension','master.hakuno-f.skill.s1','master.hakuno-f.skill.s2','master.hakuno-f.skill.s3','master.hakuno-f.skill.s4','master.hakuno-f.skill.s5','master.hakuno-m.skill.ascension','master.hakuno-m.skill.s1','master.hakuno-m.skill.s2','master.hakuno-m.skill.s3','master.rani.skill.ascension','master.rani.skill.s1','master.rani.skill.s2','master.rani.skill.s3','master.jinako.skill.ascension','master.jinako.skill.s1','master.jinako.skill.s2','master.alice.skill.ascension','master.alice.skill.s1','master.alice.skill.s2'];
    ...
    expect((byId.get('master.hakuno-f.skill.s5') as any).phase3.classificationRoute).toBe('READY_GENERIC_EXTENSION');
    const special=ids.filter(id=>id!=='master.hakuno-f.skill.s5'); for(const id of special){expect((byId.get(id) as any).phase3.classificationRoute).toBe('SPECIAL_HANDLER_CANDIDATE'); expect((byId.get(id) as any).phase3.requiredCapabilities).toContain('REVIEWED_SPECIAL_HANDLER');}
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(179); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(132);
  });
  ...
  it('keeps all five independent Card Action capabilities and tracks all grounded Card Action Activate users', () => {
    ...
    ).toEqual(['master.fujino.skill.s3','master.iliya.skill.s2','master.kariya.skill.s3','master.maiya.skill.s1','master.ryuunosuke.skill.s1','master.sakura.skill.s1','master.sakura.skill.s2','master.shinji.skill.s2']);
  });
});
