...
describe('Phase 3 full-roster capability mapping', () => {
  ...
  it('keeps the checked-in 944-identity catalog complete, fail-closed, and route-enum constrained', () => {
    ...
    expect(inventory.capabilitySummary.contractMappedCount).toBe(905);
    expect(inventory.capabilitySummary.explicitBlockCount).toBe(39);
    expect(inventory.capabilitySummary.zeroSilentFallback).toBe(true);
    expect(catalog.coverage.mappedAbilities).toHaveLength(905);
    expect(catalog.coverage.blockedAbilities).toHaveLength(39);
    ...
    expect(markdown).toContain('contractMappedCount=905');
    expect(markdown).toContain('explicitBlockCount=39');
  ...
  it('maps the ten-ID Ophelia slice with three generic extensions and seven reviewed-special event rules', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2);
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(251);
    expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(652);
  });
  ...
  it('maps the seventeen-ID Sion evidence batch as two generic and fifteen reviewed-special identities', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(251); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(652);
  });
  ...
  it('maps the fifteen-ID Archer five batch as reviewed-special identities with shared rule families', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(251); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(652);
  });
  ...
  it('maps the fifteen-ID Benkei/Billy/Boudica/Bradamante/Brynhildr batch as four generic and eleven reviewed-special identities', () => {
    ...
    expect((byId.get('servant.benkei.skill.sc-benkei-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:skill_copy_lifecycle_rule'); expect((byId.get('servant.billy.skill.sc-billy-3') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:repeat_hidden_play_draw_rule'); expect((byId.get('servant.brynhildr.skill.sc-brynhildr-2') as any).phase3.blockedBy).toContain('SPECIAL_EFFECT:love_bond_vp_rule'); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(251); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(652);
  });
