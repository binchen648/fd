...
describe('Phase 3 full-roster capability mapping', () => {
  ...
  it('keeps the checked-in 944-identity catalog complete, fail-closed, and route-enum constrained', () => {
    ...
    expect(inventory.capabilitySummary.contractMappedCount).toBe(573);
    expect(inventory.capabilitySummary.explicitBlockCount).toBe(371);
    expect(inventory.capabilitySummary.zeroSilentFallback).toBe(true);
    expect(catalog.coverage.mappedAbilities).toHaveLength(573);
    expect(catalog.coverage.blockedAbilities).toHaveLength(371);
    ...
    expect(markdown).toContain('contractMappedCount=573');
    expect(markdown).toContain('explicitBlockCount=371');
  ...
  it('maps the ten-ID Ophelia slice with three generic extensions and seven reviewed-special event rules', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(348);
  });
  ...
  it('maps the seventeen-ID Sion evidence batch as two generic and fifteen reviewed-special identities', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_EXISTING_CONTRACT).toBe(2); expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(223); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(348);
  });
  ...
  it('maps the fifteen-ID Archer five batch as reviewed-special identities with shared rule families', () => {
    ...
    expect(inventory.capabilitySummary.classificationRouteCounts.READY_GENERIC_EXTENSION).toBe(223); expect(inventory.capabilitySummary.classificationRouteCounts.SPECIAL_HANDLER_CANDIDATE).toBe(348);
  });
