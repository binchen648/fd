...
describe('Phase 3 full-roster capability mapping', () => {
  ...
  it('maps a card-play requirement to Card Action semantics without granting an acceptance contract', () => {
    ...
    const mapped = mapStructuredCapabilityNeeds(
      {
        id: 'card-play-requirement-fixture',
        printedClause: 'fixture',
        ruleModifiers: [{
    ...
    expect(mapped.requiredCapabilities).toContain('CARD_ACTION_PLAY');
    expect(mapped.specialReasons).toEqual([]);
  });
  ...
  it('keeps the checked-in 944-identity catalog complete, fail-closed, and route-enum constrained', () => {
    ...
    expect(inventory.capabilitySummary.contractMappedCount).toBe(166);
    expect(inventory.capabilitySummary.explicitBlockCount).toBe(778);
    expect(inventory.capabilitySummary.zeroSilentFallback).toBe(true);
    expect(catalog.coverage.mappedAbilities).toHaveLength(166);
    expect(catalog.coverage.blockedAbilities).toHaveLength(778);
    ...
    expect(markdown).toContain('contractMappedCount=166');
    expect(markdown).toContain('explicitBlockCount=778');
  ...
  it('closes the three-ID Araya source-evidence group with two reviewed-special identities and one existing generic extension', () => {
    const inventory = JSON.parse(
      readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'), 'utf8'),
    ) as any;
    const entries = [...inventory.staticSkills, ...inventory.dynamicSkills];
    const slice = entries.filter((entry: any) => entry.canonicalAbilityId.startsWith('master.araya.skill.'));
    const byId = new Map(slice.map((entry: any) => [entry.canonicalAbilityId, entry]));

    expect(slice).toHaveLength(3);
    for (const entry of slice) expect(entry.phase3.inheritedAcceptanceContracts).toEqual([]);
    expect(byId.get('master.araya.skill.s1a').phase3.classificationRoute).toBe('READY_GENERIC_EXTENSION');
    for (const id of ['master.araya.skill.s1', 'master.araya.skill.ascension']) {
      expect(byId.get(id).phase3.classificationRoute).toBe('SPECIAL_HANDLER_CANDIDATE');
    }
    expect(byId.get('master.araya.skill.s1').phase3.requiredCapabilities).toEqual(
      expect.arrayContaining([
        'GENERIC_CONDITION_EVALUATION',
        'GENERIC_LIFECYCLE_POLICY',
        'GENERIC_TRIGGER_GATEWAY',
    ...
    expect(byId.get('master.araya.skill.s1').phase3.blockedBy).toContain('SPECIAL_EFFECT:terrain_position_adjustment');
    expect(byId.get('master.araya.skill.ascension').phase3.requiredCapabilities).toEqual(
      expect.arrayContaining([
        'CARD_ACTION_PLAY',
        'GENERIC_CONDITION_EVALUATION',
        'GENERIC_MODIFIER',
    ...
    expect(byId.get('master.araya.skill.ascension').phase3.blockedBy).toContain('SPECIAL_EFFECT:location_token_rule');
    expect(byId.get('master.araya.skill.ascension').semanticNormalization.axes.modifier).toEqual(
      expect.arrayContaining([
  ...
  it('maps the ten-ID Rin and Shinji slice with nine generic extensions and one reviewed-special replacement rule', () => {
    const inventory = JSON.parse(
      readFileSync(resolve('data/phase3/full-roster-ability-inventory.json'), 'utf8'),
    ) as any;
    const entries = [...inventory.staticSkills, ...inventory.dynamicSkills];
    const slice = entries.filter((entry: any) =>
      entry.ownerId === 'master.rin' || entry.ownerId === 'master.shinji',
    );
    const byId = new Map(slice.map((entry: any) => [entry.canonicalAbilityId, entry]));
    ...
    expect(byId.get('master.rin.skill.s3').phase3.requiredCapabilities).toEqual(
      expect.arrayContaining([
        'CARD_ACTION_PLAY',
        'GENERIC_CARD_ZONE',
        'GENERIC_PENDING_INTERACTION',
    ...
    expect(byId.get('master.shinji.skill.s4').phase3.blockedBy).toEqual(
      expect.arrayContaining([
        'SPECIAL_EFFECT:master_identity_rule',
        'SPECIAL_EFFECT:servant_ownership_rule',
      ]),
    );
    expect(byId.get('master.shinji.skill.s4').phase3.requiredCapabilities).toEqual(
      expect.arrayContaining([
        'GENERIC_CONDITION_EVALUATION',
        'GENERIC_RESOURCE_NUMERIC',
        'GENERIC_TRIGGER_GATEWAY',
