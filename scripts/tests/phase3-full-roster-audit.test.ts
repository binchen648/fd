...
describe('Phase 3 independent full-roster automation audit', () => {
  ...
  it('keeps the checked-in real automation audit exact and independently recomputed', () => {
    ...
    expect(report).toContain('sourceEvidenceOverlayCount=472');
    expect(report).toContain('sourceEvidenceOverlayAbilityCount=705');
    expect(report).toContain('sourceGroundedCount=544');
    expect(report).toContain('semanticBlockedCount=400');
