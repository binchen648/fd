...
describe('Phase 3 full-roster semantic normalization', () => {
  ...
  it('grounds the fifteen-ID Jaguarman/Jeanne/JeanneAlter/Jekyll/Kagekiyo/Kama slice from locked development snapshots', () => {
    const overlays=loadSourceEvidenceOverlayCards();
    const ids=new Set(["servant.jaguarman.skill.sc-jaguarman-1","servant.jaguarman.skill.sc-jaguarman-2","servant.jaguarman.skill.sc-jaguarman-3","servant.jeanne.skill.sc-jeanne-1","servant.jeanne.skill.sc-jeanne-2","servant.jeanne.skill.sc-jeanne-3","servant.jeanne-alter.skill.sc-jeanne-alter-1","servant.jeanne-alter.skill.sc-jeanne-alter-2","servant.jeanne-alter.skill.sc-jeanne-alter-3","servant.jekyll.skill.sc-jekyll-1","servant.jekyll.skill.sc-jekyll-2","servant.jekyll.skill.sc-jekyll-3","servant.kagekiyo.skill.sc-kagekiyo-1","servant.kagekiyo.skill.sc-kagekiyo-3","servant.kama.skill.sc-kama-3"]);
    const slice=overlays.filter(c=>ids.has(c.id));
    expect(slice).toHaveLength(15);
    expect(slice.every(c=>c.source?.authority==='DEVELOPMENT_TEXT' && c.source.sourceText===c.printedText && c.source.sourceTextSha256===c.referencePrintedTextSha256)).toBe(true);
    ...
    expect(slice.find(c=>c.id==='servant.jeanne.skill.sc-jeanne-3')?.abilities).toContainEqual(expect.objectContaining({effects:expect.arrayContaining([expect.objectContaining({type:'defeat_player',scope:'all_combat_participants'})])}));
