...
describe('Phase 3 full-roster semantic normalization', () => {
  ...
  it('grounds the complete Chaos slice while preserving the four-option Scrambled Seals envelope', () => {
    ...
    expect(JSON.stringify(chooseOne)).toContain('ability_reuse_rule');
    expect(JSON.stringify(chooseOne)).toContain('gain_mana');
    expect(JSON.stringify(chooseOne)).toContain('gain_victory_points');
    expect(JSON.stringify(chooseOne)).toContain('move_player');
