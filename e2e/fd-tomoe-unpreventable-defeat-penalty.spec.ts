...
const hostClientId = 'host-tomoe-defeat-penalty';
const tomoePlayerId = 'p1';
const hostPlayerId = 'p6';

test('settles Tomoe unpreventable defeat penalty after scoring and preserves it across reconnect/stale replay', async ({ page, request }) => {
  const roomId = `fd-tomoe-defeat-penalty-${Date.now()}`;
  ...
  await expect.poll(() => vpOf(projections, tomoePlayerId)).toBe(10);
  ...
  await expect.poll(() => vpOf(projections, tomoePlayerId)).toBe(5);
  ...
  const tomoeResultIndex = settled.logs.findIndex((entry) =>
  ...
  expect(tomoeResultIndex).toBeGreaterThan(barrierIndex);
  ...
  expect(vpOf(projections, tomoePlayerId)).toBe(5);
...
function buildBattleSnapshot(roomId: string): MatchRoomSnapshot {
  const script = `
    ...
    const room = createMatchRoom({ roomId: ${JSON.stringify(roomId)}, hostClientId: ${JSON.stringify(hostClientId)}, hostName: '房主', seed: 20260921 });
    ...
    const definitionId = 'servant.tomoe.skill.sc-tomoe-1';
    if (!state.abilityRuntime.pack.cards[definitionId]) throw new Error('missing Tomoe skill definition');
    state.cards.push({
      instanceId: 'b21-tomoe-source',
      definitionId,
      ownerPlayerId: 'p1',
    ...
    state.abilityRuntime.cardState['b21-tomoe-source'] = { active: true, faceDown: false, playedRound: 1 };
...
function vpOf(projections: MatchRoomProjection[], playerId: string): number | undefined {
