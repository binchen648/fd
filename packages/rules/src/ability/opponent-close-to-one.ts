...
/** Exact current-main M50-02 single-opponent close-one phase-action envelope. */
export function isAcceptedOpponentCloseOneNonResidualAbility(
  ...
  } else if (!exactEmptyArray(raw, 'markers')) return false;

  const activation = node(raw.activation);
  if (!exactKeys(activation, ['phase', 'opens', 'requiresSourceState']) || activation.phase !== 'combat' ||
      activation.opens !== 'controller_combat_action_window' || activation.requiresSourceState !== 'active') return false;

  // Current-main normalizes the redundant historical phase_is condition into activation.phase.
  const conditions = nodes(raw.conditions);
  if (conditions.length !== 1) return false;
  const opponentCount = conditions[0]!;
  if (!exactKeys(opponentCount, ['type', 'scope', 'count']) || opponentCount.type !== 'target_count_equals' ||
      opponentCount.scope !== 'same_battlefield_opponents' || opponentCount.count !== 1) return false;

  for (const key of ['targets', 'cost', 'creates', 'ruleModifiers']) if (!exactEmptyArray(raw, key)) return false;
  const effects = nodes(raw.effects);
  if (effects.length !== 1 || !exactKeys(effects[0]!, ['type']) || effects[0]!.type !== OPPONENT_CLOSE_ONE_NON_RESIDUAL_EFFECT) return false;
