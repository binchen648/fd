...
describe('P3-FB2-33 event combat outcome conditions', () => {
  ...
  it('rejects event outcome conditions from choice target.conditions and disables automation', () => {
    const authored = archive({ type: 'event_player_won_combat' }) as any;
    authored.cards[0].abilities[0].conditions = [];
    authored.cards[0].abilities[0].targets = [{
      id: 'chosen_option', type: 'choice', options: [{ id: 'yes' }],
      conditions: [{ type: 'event_player_won_combat' }], count: { min: 1, max: 1 },
    }];
    const loaded = rules.loadAuthoringJson(authored);
    expect(loaded.report).toEqual(expect.arrayContaining([
      expect.objectContaining({ path: 'targets.conditions[0]', reason: 'Event combat outcome condition is supported only under ability conditions' }),
    ]));
    expect(loaded.cards['skill.source']!.abilities[0]!.execution.mode).toBe('unsupported');
  });

  it('rejects event outcome conditions from ruleModifiers.conditions and disables automation', () => {
    const authored = archive({ type: 'event_player_lost_combat' }) as any;
    ...
    ability.ruleModifiers = [{
      ...
      conditions: [{ type: 'event_player_lost_combat' }], lifecycle: { duration: 'this_round' },
    }];
    const loaded = rules.loadAuthoringJson(authored);
    expect(loaded.report).toEqual(expect.arrayContaining([
      expect.objectContaining({ path: 'ruleModifiers.conditions[0]', reason: 'Event combat outcome condition is supported only under ability conditions' }),
    ]));
    expect(loaded.cards['skill.source']!.abilities[0]!.execution.mode).toBe('unsupported');
  });
