...
describe('P3 clean-line M50 player-flag round-state core r2 replay', () => {
  ...
  it('preserves accepted finite scalar flags while numeric conditions stay safe-integer bounded', () => {
    ...
    rules.resolveEffect(state, ctx(), { type: 'set_player_flag', target: 'controller', key: 'fraction', value: 1.5 });
    expect(state.abilityRuntime!.structuredPlayerFlagsByPlayer!.p1).toEqual({ ready: true, region: 'india', uses: 2, fraction: 1.5 });
  ...
  it('rejects widened lifecycles, non-finite values, nested current_round, wrong routes, and global key/lifecycle/offset widening', () => {
    ...
    const fraction = rules.loadAuthoringJson(archive(null, [{ type: 'set_player_flag', target: 'controller', key: 'x', value: 1.5 }]));
    expect(fraction.report).toEqual([]);
    const nonFinite = rules.loadAuthoringJson(archive(null, [{ type: 'set_player_flag', target: 'controller', key: 'x', value: Infinity }]));
    expect(nonFinite.report.length).toBeGreaterThan(0);
    const unrelated = rules.loadAuthoringJson(archive(null, [{ type: 'noop', key: 'leak', lifecycle: { duration: 'this_round' }, offset: 1 }]));
    expect(unrelated.report).toEqual(expect.arrayContaining([
      expect.objectContaining({ path: 'effects[0].key', reason: 'Unmapped mechanic field' }),
      expect.objectContaining({ path: 'effects[0].lifecycle', reason: 'Unmapped mechanic field' }),
      expect.objectContaining({ path: 'effects[0].offset', reason: 'Unmapped mechanic field' }),
    ]));
  ...
  it('round-trips valid structured flag state and rejects host-signed malformed flag payloads', () => {
    ...
    session.state.abilityRuntime!.structuredPlayerFlagsByPlayer = { p1: { ready: true, label: 'ok', round, fraction: 1.5 } };
